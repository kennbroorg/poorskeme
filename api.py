    ...
    for i in abi:
        ...
        if output or input:
            ...
            if input_content != []:
                ...
                children_io.append(
                    {
                        "name": "INPUT",
                        "cssClass": "ngx-org-input-tag",
                        "image": "",
                        "title": "",
                        "childs": input_content,
                    }
                )
            if output_content != []:
                # children.append({"name": "output", "children": output_content})
                children_io.append(
                    {
                        "name": "OUTPUT",
                        "cssClass": "ngx-org-output-tag",
                        "image": "",
                        "title": "",
                        "childs": output_content,
                    }
                )
            # abi_item = {"name": head, "children": children}
        ...
        if i["type"] == "constructor":
            const.append(
                {
                    "name": "CONSTRUCTOR",
                    "cssClass": "ngx-org-constructor-node",
                    "image": "",
                    "title": "",
                    "childs": children_io,
                }
...
################################################
@app.route("/trans_info", methods=["GET"])
...
# General Transfers
################################################
@app.route("/transfer_info", methods=["GET"])
...
def r_result(address):
    ...
    if contract["native"]:
        ...
        dftemp = pd.concat(
            [dftemp_transaction, dftemp_i], join="inner", ignore_index=True
        )
    ...
    from_sum = round(detail_from["value"].sum() / 1e18, 2)
    from_count = len(detail_from)

    detail_to = df_t[df_t["to"] == address]
    to_sum = round(detail_to["value"].sum() / 1e18, 2)
    ...
    for i in detail_total.index:
        value = round(detail_total["value"][i] / 1e18, 2)
        day = detail_total["timeStamp"][i].strftime("%Y-%m-%d")
        if day_prev == "":
            day_prev = day

        if day_prev != day:
            trx_in_series.append({"name": day_prev, "value": trx_in_day})
            trx_out_series.append({"name": day_prev, "value": trx_out_day})
            day_prev = day
            day = detail_total["timeStamp"][i].strftime("%Y-%m-%d")
            trx_in_day = 0
            trx_out_day = 0
        ...
        if contract["contract"].lower() == detail_total["from"][i]:
            ...
            moves.append(
                {
                    "date": detail_total["timeStamp"][i].strftime("%Y-%m-%d %H:%M:%S"),
                    "hash": trx_hash,
    ...
    trx_in_series.append({"name": day, "value": trx_in_day})
    trx_out_series.append({"name": day, "value": trx_out_day})
    trans_vol = [
        {"name": "Trans OUT", "series": trx_out_series},
        {"name": "Trans IN", "series": trx_in_series},
    ]
