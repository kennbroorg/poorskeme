...
def eth_db_collect_async(contract_address, block_from, block_to, key, filedb, chunk=30000):
    # filedb = "contract-eth-" + contract_address + ".db"
    try:
        os.remove(filedb)
    except:
        pass
    ...
    urls_tra_creator = []
    while startblock < block_to:
        urls_transactions.append('https://api.etherscan.io/api?module=account&action=txlist&address=' + contract_address + \
                  '&startblock=' + str(startblock) + '&endblock=' + str(endblock) + '&sort=asc&apikey=' + key)
        urls_internals.append('https://api.etherscan.io/api?module=account&action=txlistinternal&address=' + contract_address + \
                  '&startblock=' + str(startblock) + '&endblock=' + str(endblock) + '&sort=asc&apikey=' + key)
        urls_transfers.append('https://api.etherscan.io/api?module=account&action=tokentx&address=' + contract_address + \
                  '&startblock=' + str(startblock) + '&endblock=' + str(endblock) + '&sort=asc&apikey=' + key)
        urls_trx_creator.append('https://api.etherscan.io/api?module=account&action=txlist&address=' + contract_creator + \
                  '&startblock=' + str(startblock) + '&endblock=' + str(endblock) + '&sort=asc&apikey=' + key)
        urls_int_creator.append('https://api.etherscan.io/api?module=account&action=txlistinternal&address=' + contract_creator + \
                  '&startblock=' + str(startblock) + '&endblock=' + str(endblock) + '&sort=asc&apikey=' + key)
        urls_tra_creator.append('https://api.etherscan.io/api?module=account&action=tokentx&address=' + contract_creator + \
                  '&startblock=' + str(startblock) + '&endblock=' + str(endblock) + '&sort=asc&apikey=' + key)
        startblock += chunk + 1
        endblock += chunk + 1
    ...
    url = 'https://api.etherscan.io/api?module=account&action=txlist&address=' + contract_creator + '&startblock=0&endblock=99999999' + \
        '&page=1&offset=1&sort=asc&apikey=' + key 
    response = requests.get(url)
    ...
    # print ("============================================================")
    # TODO: Do we need internals and ERC20?

    url_home = "https://etherscan.io/address/" + contract_creator 
    url_tokens = "https://etherscan.io/address-tokenpage?m=normal&a=" + contract_creator
    ...
    div_card_body_container = html_doc.find_all('div', class_='card-body d-flex flex-column gap-5')
    balance_container = div_card_body_container[0].find_all('div')

    # Balances
    balances = []
    # Get BNB
    # balance = balance_container[1].find_all('div')
    balance_eth = balance_container[1].text.split()[2]
    balances.append(f"Balance : {balance_eth} ETH")
    # print(f"Balance de ETH : {balance_eth}")
    ...
    token_list = div_card_body_container[0].find_all('li', class_='nav-item list-custom-ERC20')
    # print(token_list)
    for i in token_list:
    ...
    url = f'https://api.etherscan.io/api?module=account&action=txlist&address={contract_creator}' + \
    ...
    for l in contracts_list:
        url = 'https://api.etherscan.io/api?module=contract&action=getcontractcreation&contractaddresses=' + l[:-1] + \
           '&apikey=' + key
        response = requests.get(url)
